...
class MenuItem(pygame.sprite.Sprite):
    ...
    def load(self, filename, width, height, columns):
        self.master_image = pygame.image.load(filename).convert_alpha()  #加载整个菜单项图(可能包含多种状态)
        ...
        print('后2个rect:',self.image.get_rect(),self.rect) # self.image.get_rect()切割后的当前图的rect, 如(0, 0, 331, 146); self.rect切割后的当前图在整个窗口中的rect, 如(470, 80, 331, 146)
...
class MenuForm:
    def __init__(self):
        ...
        self.MENU_WIN_SIZE = (pygame.display.Info().current_w, pygame.display.Info().current_h)   # 菜单界面窗口大小
        print("menu_winsize 2:",self.MENU_WIN_SIZE)
        self.screen = pygame.display.set_mode(self.MENU_WIN_SIZE) 
        self.backgroundImage = pygame.image.load('assets/graphics/menu/menu_bg.png').convert()  #菜单窗口背景图    
